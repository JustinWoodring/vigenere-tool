...
use cursive::{
    event::Callback,
    immut2,
    utils::lines::simple::{prefix, LinesIterator, Row},
    Cursive, Rect,
};
...
pub type OnEdit = dyn Fn(&mut Cursive, &str);
...
impl TextArea {
    ...
    pub fn set_content<S: Into<Rc<String>>>(&mut self, content: S) {
    ...
    #[must_use]
    #[allow(dead_code)]
    pub fn content<S: Into<Rc<String>>>(self, content: S) -> Self {
    ...
    fn make_edit_cb(&self) -> Option<Callback> {
        self.on_edit.clone().map(|cb| {
            ...
            let content: Rc<String> = Rc::clone(&self.content);
